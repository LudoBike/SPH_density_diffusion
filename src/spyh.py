...
def sortPart(part, posSpace, partSpace, xOrigin, yOrigin, xSize, ySize, lspace, dr):
    ...
    """
    spID = sortPartTable(
        posTab, xOrigin, yOrigin, xSize, ySize, lspace, dr, maxpos0, maxpos1
    )
    part[:, SPID] = spID
    # Sort the Particles
    I = np.argsort(spID)
    ...
    if part[0, SPID] < 0:  # There exists particles outside
        ...
        print(
            repr(len(ids))
            + " particles are deleted because outside computational domain"
        )
        part = part[ids[-1] + 1 :][:]
...
def updatePartSpace(partSpace, partSPID, maxPartInSpace):
    ...
    for i in range(nSpace):
        ...
        for j in range(min(partCount, maxPartInSpace)):
...
@njit
def interpolateBoundaryPeriodicX(
    partBOUND,
    partSPID,
    partPos,
    partVel,
...
def computeForcesART(
    ...
    for i in range(nPart):
        if partFLUID[i]:
            ...
            F_visc = artViscContrib(
                mu_art, rho_i, rho_j, dwdr, rVelViscous, rPos, m, dr, eps
            )
...
def integrationStep(partFLUID, partPos, partVel, partRho, partFORCES, partDRHODT, dt):
    ...
    """
    partVel[partFLUID, :] += dt * partFORCES[partFLUID, :]
    partPos[partFLUID, :] += dt * partVel[partFLUID, :]
    partRho[partFLUID] += dt * partDRHODT[partFLUID]
    return partPos, partVel, partRho


@njit
def integrationStepPeriodicX(
    partFLUID, partPos, partVel, partRho, partFORCES, partDRHODT, dt, xper
):
    """
    Euler explicit integration step with simpletic scheme
    input :
...
@njit
def computeForcesMorrisPeriodicX(
    partFLUID,
    partSPID,
    partPos,
    partVel,
