...
from src.sphvar import BOUND, FLUID, INFO, POS, VEL


# Default figures
ftSize = 18
rc("xtick", labelsize=ftSize)
rc("ytick", labelsize=ftSize)
...
def plotPropertiesWithBound(part, partProp, nameBar, bounds, dr, PARTFIG):
    ...
    ax_list = (
        fig.axes
    )  # check if the figure is already open else get back the subplot axes
    if len(ax_list) < 1:
        ax = fig.subplots()
    else:
        ax = ax_list[0]
    coll = PolyCollection(
        vrts_fluid, array=None, facecolors=rgb, edgecolor="black", linewidths=0.1
    )
    ax.add_collection(coll)
    coll = PolyCollection(
        vrts_bound, array=None, facecolors="gray", edgecolor="black", linewidths=0.1
    )
    ax.add_collection(coll)
    ax.set_aspect("equal")
    plt.xlabel("$x$(m)", fontsize=ftSize)
    plt.ylabel("$y$(m)", fontsize=ftSize)
    plt.xlim(xMin, xMax)
    plt.ylim(yMin, yMax)
    plt.tight_layout()
    ax = plt.gca()
    ...
    cb2.set_label(nameBar, fontsize=ftSize)
    ax = plt.gca()
    ax.tick_params(axis="both", which="major", labelsize=ftSize)
    # _fig.savefig(figname,bbox_inches='tight')
...
def plotSpaces(posSpace, color, lspace, PARTFIG):
    ...
    coll = PolyCollection(
        vrts_space, array=None, facecolors="none", edgecolor=color, linewidths=1
    )
    ax.add_collection(coll)
