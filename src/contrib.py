...
def MonaghanViscContrib(mu, rho_i, rho_j, dwdr, rVel, rPos, m):
    ...
    F[:, 0] = (
        8 * mu * m / (rho_i * rho_j) * rDotPos * dwdr / rNorm**2 * rPos[:, 0] / rNorm
    )
    F[:, 1] = (
        8 * mu * m / (rho_i * rho_j) * rDotPos * dwdr / rNorm**2 * rPos[:, 1] / rNorm
    )
    return F
...
@njit
def densityDiffusionContrib(rho_i, rho_j, vol_j, rPos, dwdr, h, c0, delta):
    """
